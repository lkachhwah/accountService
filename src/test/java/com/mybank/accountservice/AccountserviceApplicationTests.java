...
class AccountserviceApplicationTests {
	...
	void contextLoads() throws Exception {
		...
		customerDetailsMapper
				.insert(CustomerDetail.builder().customerId("Lalit").emailId("laitkkac@gmai.com").name("Lk").build());
		System.out.println("USER Creation  Completed");

		AccountDetailDto accountDetailDto = accountService
				.createAccount(AccountRequestDetails.builder().balance(new BigDecimal(60)).country("India")
						.customerId("Lalit").currencies(Arrays.asList("EUR", "SEK", "USD")).build());
		...
		MvcResult result = mockMvc.perform(MockMvcRequestBuilders.get("/transaction")
				...
				.header("accountId", accountDetailDto.getAccountId())).andReturn();
